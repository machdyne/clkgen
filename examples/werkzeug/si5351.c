...
//	i2c_init();
	...
	si5351_write(SI5351_CRYSTAL_LOAD, SI5351_CRYSTAL_LOAD_10PF);
...
uint8_t si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	i2c_write_blocking(i2c_default, SI5351_BUS_BASE_ADDR, buf, bytes+1, true);
	return 0;
}
...
uint8_t si5351_write(uint8_t addr, uint8_t data)
{
	...
	buf[0] = addr;
	buf[1] = data;
	i2c_write_blocking(i2c_default, SI5351_BUS_BASE_ADDR, buf, 2, true);
	return 0;
}
...
uint8_t si5351_read(uint8_t addr, uint8_t *data)
{
	...
	buf[0] = addr;
	i2c_write_blocking(i2c_default, SI5351_BUS_BASE_ADDR, buf, 1, true);
